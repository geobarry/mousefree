...
def explorer_window():
    ...
    if not "#" in actions.user.el_prop_val(root,"class_name"):
    ...
    if el:
        ...
        if el:
            ...
            if not actions.user.element_match(winui.focused_element(),prop_list):
                ...
                if not actions.user.element_match(winui.focused_element(),prop_list):
                    actions.user.key_to_matching_element("down",prop_list,delay = 0.01,avoid_cycles = True,limit = 50,sec_lim = 1)
...
def retrieve_item_list(item_type: str = "file", ext: str = ""):
    ...
    if folder:
        ...
        items = [f for f in os.listdir(folder) if os.path.isfile(os.path.join(folder, f)) == get_files]
        if ext != '':
            n = len(ext)
            items = [item for item in items if item[-n:] == ext]
        out = {}
        id_dict = actions.user.text_to_spoken_forms(items)
        for x in id_dict.keys():
            t = x.split(" ")
        out = {x:items[id_dict[x]] for x in id_dict.keys()}
#        actions.user.explorer_select_files_panel()
        return out
...
class Actions:
    ...
    def replace_with_underscores():
        ...
        actions.insert(x)
    def explorer_tab_to_files():
        """presses the tab key to get to the file list area"""
        print("FUNCTION: file_explorer_tab_to_files")
        actions.user.key_to_matching_element("tab",[("class_name","UIItem")],limit = 35, escape_key = "esc",verbose = False)
    def explorer_select_files_panel():
    ...
    def file_explorer_copy_folder():
        ...
        actions.key("ctrl-l")
        prop_list = [("name","Address Bar"),("automation_id","TextBox")]
        root = winui.active_window().element
        el = actions.user.matching_descendant(root,prop_list,3)
        
        folder = el.value_pattern.value
        print("FUNCTION file_explorer_copy_folder")
        print(f'el: {el}')
    ...
    def explorer_open_with(app_name: str):
        ...
        if actions.user.element_match(winui.focused_element(),prop_list):
            ...
            actions.sleep(0.9)
            actions.user.key_to_matching_element("up",prop_list)
            actions.key("right")
...
#            actions.user.key_to_matching_element("tab",prop_list)
        ...
        if col_name != '':
            ...
            actions.sleep(0.1)
            root = winui.active_window().element
            print(f'root: {root}') # if we can catch this maybe we can make it quicker
            actions.sleep(0.5) # apparently this needs a long time
            prop_list = [("class_name","SysListView32")]
            root = actions.user.matching_child(root,prop_list)
            prop_list = [("name",col_name)]
    ...
    def test_current_folder():
        """test current folder for debugging"""
        folder = current_folder()
        print(f'folder: {folder}')
